...
const ENDPOINT_PLAYER_ALBUM = 'https://striveschool-api.herokuapp.com/api/deezer/album/';
...
let albumPlayer = addressBarContentPlayer.get('id_album')

const playSong = function() {
    fetch(ENDPOINT_PLAYER_ALBUM + albumPlayer)
    .then((res) => {
        console.log(res);
        if(res.ok){
            return res.json();
    ...
    .then((songsPlay) => {
        ...
        songsPlay.tracks.data.forEach((song) => {
            let oggetto = {
                title: song.title,
...
function changeSong(index) {
    songTitle.textContent = albums[index].title;
    songArtist.textContent = albums[index].artist;
    songCover.src = albums[index].cover;
    audioPlayer.src = albums[index].src;
}
...
function playNextSong() {
    currentSongIndex++;
    if (currentSongIndex >= albums.length) {
        currentSongIndex = 0;
    }
...
function playPrevSong() {
    ...
    if (currentSongIndex < 0) {
        currentSongIndex = albums.length - 1;
    }
    changeSong(currentSongIndex);
    audioPlayer.play();
}
