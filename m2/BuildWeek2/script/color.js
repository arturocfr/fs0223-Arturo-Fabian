let immAlbum = document.getElementById('immmagine_album').src;
console.log(immAlbum);
...
const draw = function (img) {
  ...
  const generateImage = function () {
    ...
    //     onload="start()"
    //     class="ms-3 shadow w-100"
    //   />`
  }
  
  const start = function () {
    // prendo il riferimento all'immagine del dom
    let imgReference = document.querySelector('#immmagine_album')
    ...
    let containerColor = document.getElementById('container-color');
    containerColor.style.backgroundColor = '#' + mostRecurrentHex
  }
